...
#'
#' \strong{Deprecated}: these accessors now live in \pkg{PhysioCore}. The
#' version here delegates to \code{PhysioCore::assaySamplingRates()} for
#' backward compatibility; new code should call the \pkg{PhysioCore} function
#' directly, or use the canonical multi-rate container
#' \code{PhysioCore::MultiRatePhysioExperiment()} when streams differ in length.
#'
#' @param x A PhysioExperiment object.
#' @return A named numeric vector of per-assay sampling rates (Hz).
#' @seealso \code{\link[PhysioCore]{assaySamplingRates}},
#'   \code{\link[PhysioCore]{MultiRatePhysioExperiment}}
#' @export
assaySamplingRates <- function(x) {
  PhysioCore::assaySamplingRates(x)
}

#' Set a per-assay sampling rate (moved to PhysioCore)
#'
#' \strong{Deprecated}: delegates to \code{PhysioCore::setAssaySamplingRate()}.
#' New code should use \pkg{PhysioCore} directly.
#'
#' @param x A PhysioExperiment object.
#' @param assay_name Name of the assay.
#' @param rate Sampling rate for the assay in Hz.
#' @return The updated \code{PhysioExperiment}.
#' @seealso \code{\link[PhysioCore]{setAssaySamplingRate}}
#' @export
setAssaySamplingRate <- function(x, assay_name, rate) {
  PhysioCore::setAssaySamplingRate(x, assay_name, rate)
}
